...
def layout(title, body, user=None, flash=None, error=None):
    ...
    if user:
        ...
        if user["role"] == "requester":
            nav.append('<a href="/authorizations">My Requests</a>')
            nav.append('<a href="/authorizations/new">New Request</a>')
            nav.append('<a href="/search">Search</a>')
        if user["role"] == "approver":
            nav.append('<a href="/authorizations">Approvals</a>')
        if user["role"] == "auditor":
            nav.append('<a href="/authorizations">All Requests</a>')
            nav.append('<a href="/audit">Audit Log</a>')
        nav_html = " ".join(nav)
        user_html = (
...
<title>{escape(title)} - JustiKey</title>
<style>
...
.muted {{ color: #64748b; }}
form.filter-form {{ margin-bottom: 0; }}
.actions {{ display: flex; gap: 10px; flex-wrap: wrap; }}
</style></head>
<body>
<header>
  <div><h1>JustiKey</h1><span class="tagline">Collect the event. Protect the identity. Require the authority. Record the access.</span></div>
  <nav>{nav_html}{user_html}</nav>
</header>
